/*
  ...
  1. Tables
    ...
    - education_records: Store education history
...
DROP INDEX IF EXISTS idx_users_settlement_site;
DROP INDEX IF EXISTS idx_applications_user_id;
DROP INDEX IF EXISTS idx_applications_status;
DROP INDEX IF EXISTS idx_applications_submitted_at;
DROP INDEX IF EXISTS idx_education_records_application_id;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_users_settlement_site') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_applications_submitted_at') THEN
    CREATE INDEX idx_applications_submitted_at ON applications(submitted_at);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_education_records_application_id') THEN
    CREATE INDEX idx_education_records_application_id ON education_records(application_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_documents_application_id') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'applications_search_idx') THEN
...
DROP TRIGGER IF EXISTS update_education_records_timestamp ON education_records;
...
CREATE POLICY "Users can view own education records"
  ON education_records FOR SELECT
  TO authenticated
  USING (EXISTS (
    SELECT 1 FROM applications
    WHERE applications.id = education_records.application_id
